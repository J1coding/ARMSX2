...
struct GameScreenView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: childPresentedBinding(.framePacing)) {
            NavigationStack {
    ...
    private func openPauseMenuChild(_ destination: QuickMenuDestination) {
        ...
        case .speed, .framePacing, .saveStates, .cheats, .retroAchievements, .padLayout, .resetROM:
    ...
    private func presentRetroAchievementsToast(_ notification: Notification) {
        ...
        // Honor the per-event duration the bridge passes through (seconds) instead of the
        // banner default, so longer popups (e.g. mastery) and shorter ones are respected.
