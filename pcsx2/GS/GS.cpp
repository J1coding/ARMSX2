...
void* GSAllocateWrappedMemory(size_t size, size_t repeat)
{
	...
	// Route through HostSys::CreateSharedMemory so the existing iOS file-backed
	// fallback in common/Linux/LnxHostSys.cpp applies. Without this, the bare
	// shm_open("/GS.mem", ...) the prior implementation used is rejected by the
	// iOS sandbox and GSLocalMemory::GSLocalMemory() aborts via
	// pxFailRel("Failed to allocate GS memory storage."). See
	// LiveContainer-2026-07-18-153408.ips (UUID 5F29F60B-A11F-3D7E-85F5-8F49FF22C9CD).
